...
int CFileMaker::InitOutputFile(
	const char* pFileOut,
	const tAudioInputParams* pAudioParams,
	int* outAudioIndex)
{
	int				nReturn;
	AVStream*		pStream;
	...
	if (pAudioParams)
	{
		...
		if (outAudioIndex) *outAudioIndex = pStream->id;
...
int CFileMaker::Open(const char* pFileOut, 
	const tAudioInputParams* pAudioParams, 
	int* outAudioIndex)
{
	if (outAudioIndex)
...
{ 
	...
	if (m_arrayFilterCtx)
	{
		for (unsigned int i = 0; i < nStreamNum; i++)
		{
			if (m_arrayFilterCtx[i].pFilterCtxSink) avfilter_free(m_arrayFilterCtx[i].pFilterCtxSink);
			if (m_arrayFilterCtx[i].pFilterCtxSrc)	avfilter_free(m_arrayFilterCtx[i].pFilterCtxSrc);
			if (m_arrayFilterCtx[i].pFilterGraph)	avfilter_graph_free(&m_arrayFilterCtx[i].pFilterGraph);
		}
...
int CFileMaker::InitFilterInner(tFilteringContext* pFilterCtx, AVCodecContext* pEncodeCtx, const char* pFilterSpec)
{
	...
	if (pEncodeCtx->codec_type == AVMEDIA_TYPE_VIDEO)
	{
		...
		_snprintf_s(args, sizeof(args), "video_size=%dx%d:pix_fmt=%d:time_base=%d/%d:pixel_aspect=%d/%d",
			...
			pEncodeCtx->height, 
			pEncodeCtx->pix_fmt,
			pEncodeCtx->time_base.num, 
			pEncodeCtx->time_base.den,
	...
	else if (pEncodeCtx->codec_type == AVMEDIA_TYPE_AUDIO)
	{
		...
		_snprintf_s(args, sizeof(args), "time_base=%d/%d:sample_rate=%d:sample_fmt=%s:channel_layout=0x%I64x",
			...
			av_get_sample_fmt_name(pEncodeCtx->sample_fmt),
		...
		ret = av_opt_set_bin(buffersink_ctx, "channel_layouts",	(uint8_t*)&pEncodeCtx->channel_layout, sizeof(pEncodeCtx->channel_layout), AV_OPT_SEARCH_CHILDREN);
		if (ret < 0)	goto end;
 
		ret = av_opt_set_bin(buffersink_ctx, "sample_rates", (uint8_t*)&pEncodeCtx->sample_rate, sizeof(pEncodeCtx->sample_rate), AV_OPT_SEARCH_CHILDREN);
		if (ret < 0)	goto end;
	} 
	else 
	{
...
int CFileMaker::InsertFrame(AVFrame *pFrame, int nStreamIndex)
{
	...
	if (!m_pFormatCtxOutput ||
		nStreamIndex < 0	||
		(unsigned int)nStreamIndex >= m_pFormatCtxOutput->nb_streams ||
		!m_arrayFilterCtx ||
...
int CFileMaker::EncodeWriteFrame(AVFrame *pFiltFrame, unsigned int nStreamIndex, int *pGetPkt) 
{
	...
	if (!pGetPkt) pGetPkt = &got_frame_local;
