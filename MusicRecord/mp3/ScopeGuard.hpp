...
public:
	...
	{
		Execute<N - 1>::execute(f, std::forward<T>(t), std::get<N - 1>(std::forward<T>(t)),
					p...);
	}
};
...
public:
	static ScopeGuard<F, P...> MakeScopeGurad(F f, P... p)
	{
		return ScopeGuard(std::move(f), std::move(p)...);
	}
...
public:
	template<class O, typename F, typename T, typename... P>
	static void execute(O &&o, F &&f, T &&t, P &&...p)
	{
		ObjectExecute<N - 1>::execute(o, f, std::forward<T>(t),
					      std::get<N - 1>(std::forward<T>(t)), p...);
	}
};
...
public:
	static ObjectScopeGuard<O, F, P...> MakeObjectScopeGurad(O &&o, F f, P... p)
	{
		return ObjectScopeGuard(std::move(o), std::move(f), std::move(p)...);
	}

	ObjectScopeGuard(O &&o, F &&f, P &&...p) throw()
		: object_(o), function_(f), parameter_(p...), dismissed_(false){};

	~ObjectScopeGuard()
	{
		if (!dismissed_) {
			try {
				ObjectExecute<sizeof...(P)>::execute(object_, function_,
								     parameter_);
			} catch (...) {
			}
		}
...
template<class O, typename F, typename... P>
inline auto MakeObjectGurad(O &&o, F f, P... p) -> ObjectScopeGuard<O, F, P...>
{
